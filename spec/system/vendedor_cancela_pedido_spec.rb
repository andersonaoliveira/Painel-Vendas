...
describe 'Vendedor visualiza tela de pedidos' do
  it 'e veê a tela de cancelamento' do
    # Arrange
  ...
  it 'e não consegue cancelar o pedido de outro vendedor' do
    # Assert
    seller = create(:user, name: 'Jim Halpert', role: :salesperson, email: 'jimhalpert@locaweb.com.br')
    seller2 = create(:user, name: 'Dwight Schrute', role: :salesperson, email: 'dwightschrute@locaweb.com.br')
    client = create(:client)
    plan = Plan.new(id: 1, name: 'Plano X', product_group: 'Hospedagem')
    price = Price.new(id: 1, period: 'Mensal', value: '50')
    ...
    # Act
    login_as(seller2)
    visit review_cancelation_order_path(order.id)
